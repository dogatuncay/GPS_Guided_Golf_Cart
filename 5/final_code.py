...
#GPS FOLLOWER
	...
	current_coordinate2=[self.course[(self.waypoint)%len(self.course)][1],self.course[(self.waypoint)%len(self.course)][2]]

	current_coordinate22=[self.course[(self.waypoint+1)%len(self.course)][1],self.course[(self.waypoint+1)%len(self.course)][2]]
#calculate the distance

	d=distance.distance(current_coordinate2,current_coordinate22).kilometers * 1000
	
	distance_left = distance.distance(current_coordinate22,current_coordinate).kilometers*1000
...
#Here we use forward_azimuth of geopy
	...
	print  fabs(angle_obs-angle_cur),  calc_angle
	...
	compass1=distance_wpp
	compass2=distance_wpn
#tactical maneuvers
	...
	angle = atan(difference_y_axis/difference_x_axis)
	if angle < 0:
		angle = angle + 360
	rad = radians(angle)
	...
	if self.obs_found == 'F':
		if ( d>20 and self.syntpointdone=='F'):
			...
			diff_between_current_and_synth=distance.distance(current_coordinate,syntpoint_latlon).kilometers * 1000
			if(diff_between_current_and_synth<=3):
...
# adding the synthetic points by adding a small constant c to be able to pass the obstacle without hitting		
		if  self.obstacle[self.obs_count][1][0] >= self.course[(self.waypoint+1)%len(self.course)][1]:
			if self.obstacle[self.obs_count][1][1] <= self.course[(self.waypoint+1)%len(self.course)][2]:			
				self.synt_latlon = [self.obstacle[self.obs_count][1][0] + .000014, self.obstacle[self.obs_count][1][1] + .000014]
			else:
				self.synt_latlon = [self.obstacle[self.obs_count][1][0] + .000014, self.obstacle[self.obs_count][1][1] - .000014]
		else:
			if self.obstacle[self.obs_count][1][1] <= self.course[(self.waypoint+1)%len(self.course)][2]:			
				self.synt_latlon = [self.obstacle[self.obs_count][1][0] - .000014, self.obstacle[self.obs_count][1][1] + .000013]
			else:
				self.synt_latlon = [self.obstacle[self.obs_count][1][0] - .000014, self.obstacle[self.obs_count][1][1] - .000014]
		
		self.waypoint_latlon = self.synt_latlon
		temp = [self.syntpoint[self.syntpointnum][0],self.syntpoint[self.syntpointnum][1]]
		diff_between_current_and_synth=distance.distance(current_coordinate,temp).kilometers * 1000

		if(diff_between_current_and_synth<5):
	...
	meters_to_target = distance.distance(current_coordinate,self.waypoint_latlon).kilometers * 1000
    	heading_to_target = distance.distance(current_coordinate,self.waypoint_latlon).forward_azimuth


        # Calcuate the next steering input
        turn, diff = self.calc_inv_turn(heading_to_target)

        # Whether we go straight or not
        if turn_state == 'turn':
